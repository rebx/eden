...
class SingleCharacterTokenizationTest < Test::Unit::TestCase
  ...
  def test_single_character_tokenisation
    ...
    assert_equal :semicolon, line.tokens[19].type
    assert_equal "=", line.tokens[20].content
    assert_equal :equals, line.tokens[20].type
    assert_equal "?", line.tokens[21].content
    assert_equal :question_mark, line.tokens[21].type    
    assert_equal "+", line.tokens[22].content
    assert_equal :plus, line.tokens[22].type    
    assert_equal "-", line.tokens[23].content
    assert_equal :minus, line.tokens[23].type    
  end
